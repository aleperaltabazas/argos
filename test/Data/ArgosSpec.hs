...
spec = describe "ArgosSpec" $ do
  describe "spread" $ do
    it "returns the long option with two dashes at the beginning" $ do
      spread Option { long = "foo", short = Nothing, argument = Nothing } `shouldBe` Map.fromList [(0, [Layer Nothing "--foo"])]
      spread Option { long = "bar", short = Nothing, argument = Nothing } `shouldBe` Map.fromList [(0, [Layer Nothing "--bar"])]
    it "returns the long option with two dashes at the beginning and the short option with one" $ do
      spread Option { long = "foo", short = Just 'f', argument = Nothing }
        `shouldBe` Map.fromList [(0, [Layer Nothing "--foo -f"])]
      spread Option { long = "bar", short = Just 'b', argument = Nothing }
        `shouldBe` Map.fromList [(0, [Layer Nothing "--bar -b"])]
    ...
    it "returns a map of length two" $ do
      spread Command { name = "foo", arguments = [Option { long = "bar", short = Nothing, argument = Nothing }] }
      ...
      spread Command
          { name      = "foo"
          , arguments =
            [ Option { long = "bar", short = Nothing, argument = Nothing }
            , Option { long = "baz", short = Nothing, argument = Nothing }
            ]
          }
          ...
          , arguments =
            [ Command
              ...
              , arguments =
                ...
                , Option { long = "biz", short = Nothing, argument = Nothing }
                , Command { name = "qux", arguments = [Option { long = "corge", short = Nothing, argument = Nothing }] }
                ]
              }
            , Command { name = "grault", arguments = [Option { long = "garply", short = Nothing, argument = Nothing }] }
            ]
          }
